...
/// Read a `.pid` into the same outcome the DWG and DXF readers return.
///
/// `ReadStats` describes a record stream, which a `.pid` does not have -- the
/// import walks `pid-parse`'s evidence list instead. What the open path asks
/// of these numbers is only whether the read produced anything
/// (`has_usable_drawing_data`) and whether it limped: a non-zero
/// `recovered_errors` or `skipped_source_records`, or an unfinished stream,
/// sends the strict attempt to the recovery prompt. So they report the import
/// as the clean, complete read it is. `load_pid` has already refused the file
/// if nothing source-backed decoded, and what it could not draw is named in
/// its own import report rather than arriving here as reader damage.
///
/// A failure is terminal: re-reading a `.pid` in DWG failsafe mode has nothing
...
fn read_pid_path(path: &Path) -> Result<acadrust::ReadOutcome, ReaderFailure> {
    ...
    let stats = acadrust::ReadStats {
        ...
        // is also what keeps `finalize_loaded_outcome`'s DXF-only repairs off
        // it.
        source_format: None,
        record_stream_read: true,
        stream_completed: true,
        source_records: entities,
        decoded_source_records: entities,
        output_entities: entities,
        ..Default::default()
    };
    Ok(acadrust::ReadOutcome::new(document, stats))
}
